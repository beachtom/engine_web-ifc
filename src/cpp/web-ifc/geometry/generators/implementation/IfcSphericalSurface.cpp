...
namespace webifc::geometry::generators {
	std::vector<glm::dvec3> GenerateIfcSphericalSurface(const uint32_t expressID, const uint32_t, webifc::parsing::IfcLoader &loader, webifc::cache::IfcCache &cache, GeometryGeneratorSettings &settings, const bool cacheOff) {
		if (!cacheOff) {
