...
namespace webifc::geometry::generators {
	std::vector<glm::dvec3> GenerateIfcCartesianPointList3D(const uint32_t expressID, const uint32_t lineType, webifc::parsing::IfcLoader &loader, webifc::cache::IfcCache &cache, GeometryGeneratorSettings &settings, bool cacheOff) {
		if (!cacheOff) {
