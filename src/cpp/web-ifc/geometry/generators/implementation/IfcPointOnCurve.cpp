...
namespace webifc::geometry::generators {
	glm::dvec3 GenerateIfcPointOnCurve(const uint32_t expressID, const uint32_t, webifc::parsing::IfcLoader &loader, webifc::cache::IfcCache &cache, GeometryGeneratorSettings &settings, const bool cacheOff) {
		if (!cacheOff) {
